...
module.exports = async (options, configItem) => {
    ...
    // 获取需要参数
    let initDcResult = await setHeaders(
        superagent.post('https://kyfw.12306.cn/otn/confirmPassenger/initDc')
    ).send({
        _json_att: ''
    });

    const globalRepeatSubmitToken = /var globalRepeatSubmitToken = '(.*)';/.exec(
        initDcResult.text
    )[1];
    ...
    let currentUser =
        config.userList[options.userIndex === undefined ? config.userIndex : options.userIndex];

    // 提交订单
    let checkOrderInfoResult = await setHeaders(
        superagent.post('https://kyfw.12306.cn/otn/confirmPassenger/checkOrderInfo')
    ).send({
        cancel_flag: '2',
    ...
    // 查询余票
    let getQueueCountResult = await setHeaders(
        superagent.post('https://kyfw.12306.cn/otn/confirmPassenger/getQueueCount')
    ).send({
    ...
    // 确认座位
    let confirmSingleForQueueResult = await setHeaders(
        superagent.post('https://kyfw.12306.cn/otn/confirmPassenger/confirmSingleForQueue')
    ).send({
        passengerTicketStr: queryParams.seatType + currentUser.passengerTicketStr,
        oldPassengerStr: currentUser.oldPassengerStr,
        randCode: '',
    ...
    await (async function () {
        ...
        let queryOrderWaitTimeResult = await setHeaders(
            superagent.get('https://kyfw.12306.cn/otn/confirmPassenger/queryOrderWaitTime')
        ).query({
            random: Date.now(),
            tourFlag: 'dc',
            _json_att: '',
    ...
    // 下单结果查询
    let resultOrderForDcQueueResult = await setHeaders(
        superagent.post('https://kyfw.12306.cn/otn/confirmPassenger/resultOrderForDcQueue')
    ).send({
